...
def createTeam(firstIndex, secondIndex, isRed,
               first = 'StrategyAgent', second = 'StrategyAgent'):
  """
...
class StrategyAgent(CaptureAgent):
  ...
  def gotoPosList(self, gameState, pos):
    ...
    if x+1<self.w:
      if not self.wall[x+1][y]:
        tmp = self.getMazeDistance((x+1, y),pos)
        if tmp < distance:
          if (self.isRed and x+1>=self.w/2) or (not self.isRed and x+1<self.w/2):
            if self.SafeArea[x+1][y]<0:            
